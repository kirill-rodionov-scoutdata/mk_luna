...
@pytest.mark.asyncio
async def test_process_payment_succeeds(payment_service, outbox_service, db_session, payment_records):
    ...
    with patch.object(outbox_service, "_simulate_external_gate_processing", AsyncMock()):
        with patch.object(outbox_service, "send_webhook_notification", AsyncMock()) as mock_webhook:
...
@pytest.mark.asyncio
async def test_process_payment_fails_on_gateway_error(payment_service, outbox_service, db_session, payment_records):
    # Setup
    record = payment_records[0]
    kwargs = {**record, "amount": Decimal(record["amount"]), "currency": Currency(record["currency"])}
    payment = await payment_service.create_payment(**kwargs)
    
    # Mock simulation to fail
    with patch.object(outbox_service, "_simulate_external_gate_processing", AsyncMock(side_effect=Exception("Gateway error"))):
...
@pytest.mark.asyncio
async def test_process_payment_idempotent_if_already_processed(payment_service, outbox_service, db_session, payment_records):
    ...
    with patch.object(outbox_service, "_simulate_external_gate_processing", AsyncMock()) as mock_sim:
        await outbox_service.process_payment(payment.id)
